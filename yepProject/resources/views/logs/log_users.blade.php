...
<div class="container-fluid">
    <h5>{{ __('strings.lb_settings') }} &gt; {{ __('strings.lb_options_manage') }} &gt; {{ __('strings.lb_logs_users') }}</h5>
    <div class="mt-3">
        ...
        <table class="mt-3 table table-striped">
            ...
            @foreach($data as $datum)
                <tr class="text-center">
                    ...
                    <td>
                        @if (!is_null($datum->writer))
                            {{ $datum->writer->name }}
                        @else
                            {{ __('strings.err_deleted_data') }}
                        @endif
                    </td>
                    <td>{{ strtoupper($datum->field_name) }}</td>
