...
class SmsJobController extends Controller
{
    ...
    public function getSmsSendResult(Request $request){
        ...
        foreach($data as $datum){
            $udate = date("Y-m-d H:i:s",strtotime($datum->updated_at));
    ...
    public function SmsCheckSameTestId(Request $request){
        ...
        $cnts = SmsPapers::select(DB::raw('count(*) as tf_id'))->whereIn('id',$pidArray)->distinct()->groupBy('tf_id')->get()->count();
        if ($cnts <= 0){
    ...
    public function SmsMergedExcelDownload($sper){
        ...
        $papers = SmsPapers::whereIn('id',$sperArray)->distinct()->groupBy('tf_id')->latest();
        dd($papers);
